...
TError CTextTexture::Init  (CDisplayDevice *pDev)
{
  ...
  HRESULT hr;
  hr = pDev->GetDirect3DDevice()->CreateVertexBuffer(4*sizeof(TTLVertexUV), D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, DV_FVF_TLVERTEXUV, D3DPOOL_DEFAULT, &m_pVBuffer, NULL);
  if (FAILED(hr))
    return RET_FAIL;
...
//      if (curTexW < (int)m_pDev->GetMaxTextureW())
  ...
  if (size.cx && size.cy && !FAILED(m_pDev->GetDirect3DDevice()->CreateTexture(maxTexW, 256, 1, 0, D3DFMT_A4R4G4B4, D3DPOOL_MANAGED, &m_pTex, NULL)))
  {
...
void  CTextTexture::Draw        (CRenderContext &rc, TColor c, float x, float y)
{
  ...
  TTLVertexUV *pav = NULL;
  HRESULT hr = m_pVBuffer->Lock(0, 0, reinterpret_cast<void**>(&pav), D3DLOCK_DISCARD | D3DLOCK_NOSYSLOCK);
  if (!FAILED(hr))
  {
    pav[0].Set(aad+x,   aad+y,   z, rhw, c, 0,                        0);
    pav[1].Set(aad+x+w, aad+y,   z, rhw, c, float(m_extentX)/m_texW,  0);
    pav[2].Set(aad+x,   aad+y+h, z, rhw, c, 0,                        float(m_extentY)/256);
    pav[3].Set(aad+x+w, aad+y+h, z, rhw, c, float(m_extentX)/m_texW,  float(m_extentY)/256);

    m_pVBuffer->Unlock();

//    mat.Set(rc.GetDevice());
    CDisplayShader Shader;
    Shader.Init();
    Shader.GetPass(0)->SrcBlend = D3DBLEND_SRCALPHA;
    Shader.GetPass(0)->DstBlend = D3DBLEND_INVSRCALPHA;
    Shader.GetPass(0)->ZWrite = false;
    Shader.GetPass(0)->ZFunc = D3DCMP_ALWAYS;
//    Shader.GetStage(0, 0)->pTex = m_pTex;
    Shader.Set(rc.GetDevice());

    rc.GetDevice()->SetTexture(0, m_pTex);

    rc.GetDevice()->SetFVFShader(DV_FVF_TLVERTEXUV);
    rc.GetDevice()->GetDirect3DDevice()->SetStreamSource(0, m_pVBuffer, 0, sizeof(*pav));
    rc.GetDevice()->GetDirect3DDevice()->DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, 2);
